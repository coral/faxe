...
assert.deepEqual((await readdir(dist)).sort(), [
  ...
  'privacy.html', 'robots.txt', 'screenshot_mac.png', 'styles.css',
]);
...
for (const name of ['index.html', '404.html', 'licenses.html', 'privacy.html']) {
  const html = await readFile(new URL(name, dist), 'utf8');
  assert.ok(!html.includes('{{'), `Unrendered template in ${name}`);
  for (const [, url] of html.matchAll(/(?:href|src)="(\/[^"#]*)"/g)) {
    const pages = { '/': 'index.html', '/licenses': 'licenses.html', '/privacy': 'privacy.html' };
    const file = pages[url] ?? url.slice(1);
